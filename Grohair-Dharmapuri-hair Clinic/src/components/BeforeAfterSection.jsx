...
function WipeImg({ src, alt, exitDir, objPos = 'center center' }) {
  ...
  const outAnim = exitDir === 'right' ? 'wipeOutRight 0.2s ease forwards' : 'wipeOutLeft 0.2s ease forwards'
  const inAnim = exitDir === 'right' ? 'wipeInLeft 0.2s ease forwards' : 'wipeInRight 0.2s ease forwards'
        ...
        {pairs.map((_, i) => (
          <button
            ...
            className={`rounded-full transition-all duration-300 ${
              i === idx ? 'w-4 h-2 bg-red-700' : 'w-2 h-2 bg-gray-200'
            }`}
